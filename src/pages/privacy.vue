...
<template>
  <div class="protocol">
    ...
    通过移动应用使用{{ appName }}时，可能涉及我们对您信息的收集与使用。本隐私政策向您说明并告知，当您通过我们的移动应用访问和使用我们的服务时，{{ appName }}如何收集、使用和披露您的信息。
    ...
    <br><br>
    1.3 所收集的任何信息均适用信息收集时生效的隐私政策。但我们可能会不时修改和修订本隐私政策。若我们对本政策作出重大变更，无论是否另行通知，您均同意受修订后的政策约束。
    <br><br>
    ...
    <br><br>
    4）支付信息，例如银行账号、贝宝（PayPal）或其他待支付相关信息；
    <br><br>
    5）您购买付费服务后的交易信息，例如交易编号、交易记录；
    <br><br>
    6）您的授权选择及与我们的沟通信息，例如用于验证您账号、处理您的反馈或投诉的信息，以及您参与应用内问卷调查和活动时提供的信息；
    <br><br>
    7）您通过关联第三方社交媒体账号（如脸书、谷歌）注册{{ appName }}账号时，从社交媒体授权提供的信息；
    <br><br>
    2. 基于合法权益自动收集的信息
    <br><br>
    ⊙ 您在其他服务上的账号相关数据：若您的社交媒体或其他网络账号与我们服务中的账号相关联，我们可能通过这些账号获取部分信息。若您通过脸书或其他第三方平台登录我们的服务，我们会请求您授权访问该账号的相关信息。例如，根据不同平台或服务，我们可能收集您的姓名、头像、性别、出生日期等信息，其中谷歌相关信息包括：谷歌账号唯一标识（ID）、身份令牌；脸书相关信息包括：访问令牌。此类平台和服务通过其应用程序接口向我们提供信息，我们收到的信息取决于您（通过隐私设置）决定向我们提供的内容，或相关平台、服务的设定。若您通过第三方平台或服务访问、使用我们的服务，或点击任何第三方链接，您数据的收集、使用和共享同样受该第三方隐私政策及其他协议约束。
    <br><br>
    ⊙ 设备标识信息，例如您的操作系统、浏览器类型、品牌、型号、移动设备序列号、互联网协议（IP）地址、移动运营商、屏幕分辨率、语言设置、国际移动设备识别码（IMEI）、国际移动用户识别码（IMSI）以及媒体访问控制地址；
    <br><br>
    ...
    <br><br>
    2.3 未成年人信息特别说明 {{ appName }}服务仅面向年满18周岁及以上用户。我们不允许18周岁以下用户访问本平台，也不会故意收集18周岁以下用户的个人信息。
    <br><br>
    <div class="bt">
      3. 我们如何使用所收集的信息
    </div>
    ...
    <br><br>
    1）第三方使用分析工具：我们使用谷歌公司（美国加利福尼亚州山景城环形大道1600号，邮编94043）提供的工具进行服务使用情况分析。我们使用谷歌旗下的Firebase工具统计使用数据，该工具同样用于使用情况分析。
    <br><br>
    2）第三方转化追踪工具：使用AppsFlyer、谷歌转化追踪接口、脸书转化追踪接口进行用户转化追踪。
    <br><br>
    3）崩溃检测工具：我们使用谷歌旗下Firebase的Crashlytics工具进行崩溃检测，有助于维护和优化服务。您的所有崩溃数据均以匿名方式收集。
    <br><br>
    3.4 您的信息共享
    <br><br>
    ...
    <br><br>
    2）我们可能向{{ appName }}集团旗下合法关联企业共享用户内容及您的信息（包括但不限于Cookie信息、日志文件、设备标识、比特数据及使用数据）。关联企业可使用此类信息协助提供、理解和优化服务（包括提供分析服务）及自身服务（为您提供更优质、更贴合需求的体验），但此类关联企业将尊重您对内容可见范围的设置选择。
    <br><br>
    3）我们还可能向协助我们为您提供服务的第三方机构（服务提供商）共享您的信息，以及Cookie、日志文件、设备标识、位置数据等工具获取的信息。我们的服务提供商将在合理保密条款约束下，访问您的信息以提供对应服务。
    <br><br>
    ...
    <br><br>
    7）若您删除在服务中创建的信息和内容（如截图等），服务缓存和存档页面中可能仍保留副本；若其他用户或第三方（如脸书等）已复制或保存相关信息，也可能继续可见。
    <br><br>
    3.5 我们为保护您信息采取的安全措施
    <br><br>
    ...
    本服务并非针对未满18周岁的未成年人设计，我们不会故意接受或向未满18周岁未成年人收集、出售个人可识别信息作为服务内容。若我们发现无意中收到未满18周岁用户的个人可识别信息，将从记录中删除该信息。若未来我们调整相关规则，在收集未满18周岁未成年人的个人可识别信息前，将事先获得可验证的监护人同意。
    <br><br>
    <div class="bt">
      8. 政策变更通知
    </div>
